...
namespace nh
{
	...
	bool ModelComponent::Read(const rapidjson::Value& value)
	{
		...
		model = owner->scene->engine->Get<nh::ResourceSystem>()->Get<nh::Model>(model_name);

		std::string material_name;
		JSON_READ(value, material_name);
		material = owner->scene->engine->Get<nh::ResourceSystem>()->Get<nh::Material>(material_name, owner->scene->engine);
